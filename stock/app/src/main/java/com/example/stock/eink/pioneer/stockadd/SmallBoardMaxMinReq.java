...
public class SmallBoardMaxMinReq extends HttpRequest {
    ...
    public SmallBoardMaxMinReq(int startCode,String startDatePt){
        ...
        saver = new YearMaxMinPriceDataSave(db,"smallboard");
    }
    ...
    public void startReq() {
        ...
        while (requestState){
            ...
            System.out.println("SmallBoardMaxMinReq executeIndexId="+Integer.toString(executeIndexId));
    ...
    private void InitReqStockCode(){
        String codeTemp = "sz" + getFullStockCode(startReqCode);
        Cursor cursor = db.query("smallboard", new String[] {"id"},"code=?",
        ...
        cursor = db.query("smallboard", new String[] {"MAX(id)"},null,
                null, null, null, null);
        // 将光标移动到下一行，从而判断该结果集是否还有下一条数据，如果有则返回true，没有则返回false
        while (cursor.moveToNext()) {
        ...
        System.out.println("SmallBoardMaxMinReq start executeIndexId="+Integer.toString(executeIndexId));
        System.out.println("SmallBoardMaxMinReq start executeReqCode="+executeReqCode);
        System.out.println("SmallBoardMaxMinReq table tableMaxIndex="+Integer.toString(tableMaxIndex));
    }

    private void obtainReqStockCode(){
        executeReqCode = "";

        Cursor cursor = db.query("smallboard", new String[] {"code"},"id=?",
    ...
    private String getFullStockCode(int data){
        String tmpStr = Integer.toString(data);
        if(tmpStr.length() == 1){
            tmpStr = "00000" + tmpStr;
